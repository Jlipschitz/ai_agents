...
function getConfiguredAgentIds(config) {
  return Array.isArray(config.agentIds) && config.agentIds.length ? config.agentIds : DEFAULT_AGENT_IDS;
}
...
function createStarterConfig(configPath) {
  const starter = {
    ...
    paths: {
      sharedRisk: ['scripts', 'package.json', 'agent-coordination.config.json'],
      visualSuite: [],
      visualSuiteDefault: [],
      visualImpact: [],
      visualImpactFiles: [],
    ...
    notes: {
      categories: ['error', 'inconsistency', 'change', 'gotcha', 'decision', 'verification', 'setup'],
      sectionHeading: 'Agent-Maintained Notes',
    },
    pathClassification: {
      productPrefixes: ['app', 'src', 'components', 'features', 'packages'],
      dataPrefixes: ['api', 'db', 'database', 'hooks', 'lib', 'migrations', 'server', 'store', 'types'],
      verifyPrefixes: ['tests', 'test', '__tests__', 'spec'],
      docsPrefixes: ['docs', 'scripts'],
      docsFiles: ['README.md', 'agent-coordination.config.json', 'package.json'],
    },
    planning: {
      defaultDomains: ['app'],
      productFallbackPaths: ['app', 'src', 'components', 'features'],
      dataFallbackPaths: ['api', 'lib', 'server', 'types'],
      verifyFallbackPaths: ['tests'],
      docsFallbackPaths: ['README.md', 'docs'],
      agentSizing: {
        minAgents: 1,
        maxAgents: 4,
    ...
    domainRules: [
      {
        ...
        scopes: { product: ['app', 'src', 'components', 'features'], data: ['lib', 'hooks', 'store', 'types'], verify: ['tests'], docs: ['README.md', 'docs'] },
      },
    ],
  };
...
function doctorFix() {
  ...
  if (!fs.existsSync(configPath)) {
    ...
    fixes.push(`created ${normalizePath(configPath)}`);
  }

  if (appendUniqueLines(path.join(ROOT, '.gitignore'), ['', '# Local AI agent coordination runtime state', '/coordination/', '/coordination-two/'])) {
    fixes.push('updated .gitignore');
  }
  ...
  for (const dir of [paths.coordinationRoot, paths.tasksRoot, paths.runtimeRoot, paths.heartbeatsRoot]) {
    if (!fs.existsSync(dir)) {
      ...
      fixes.push(`created ${normalizePath(dir)}`);
    }
  }

  if (!fs.existsSync(paths.boardPath)) {
    writeJson(paths.boardPath, { version: 1, projectName: path.basename(ROOT), tasks: [], resources: [], incidents: [], updatedAt: new Date().toISOString() });
    fixes.push(`created ${normalizePath(paths.boardPath)}`);
  }
...
function getGitSnapshot() {
  ...
  if (result.upstream) {
    try {
      const [ahead, behind] = git(['rev-list', '--left-right', '--count', `${result.upstream}...HEAD`]).split(/\s+/).map((value) => Number.parseInt(value, 10));
  ...
  try {
    ...
    for (const line of porcelain.split(/\r?\n/).filter(Boolean)) {
      ...
      if (line.startsWith('??')) result.untracked.push(filePath);
      else result.dirty.push(filePath);
    }
  } catch {}

  const gitDir = (() => {
    try { return git(['rev-parse', '--git-dir']); } catch { return null; }
  })();
  if (gitDir) {
...
function buildBoardSummary({ forChat = false } = {}) {
  ...
  if (forChat) {
    return [
      `Coordination summary for ${board.projectName || path.basename(ROOT)}`,
      `Updated: ${updatedAt}`,
      `Active: ${active.length}; Blocked: ${blocked.length}; Review: ${review.length}; Planned: ${planned.length}; Done/Released: ${done.length}`,
      active.length ? `Active work:\n${active.map(taskSummary).join('\n')}` : 'Active work: none',
  ...
  return [
    `# Board Summary`,
    '',
    `Project: ${board.projectName || path.basename(ROOT)}`,
    `Coordination root: ${normalizePath(coordinationRoot) || coordinationRoot}`,
    `Updated: ${updatedAt}`,
    '',
    `## Counts`,
    '',
    `- Planned: ${planned.length}`,
    `- Active-like: ${active.length}`,
    `- Blocked: ${blocked.length}`,
    `- Review: ${review.length}`,
    `- Done/released: ${done.length}`,
    '',
    `## Active Work`,
    '',
    active.length ? active.map(taskSummary).join('\n') : '- None',
    '',
    `## Blockers`,
    '',
    blocked.length ? blocked.map(taskSummary).join('\n') : '- None',
    '',
    `## Review Queue`,
    '',
    review.length ? review.map(taskSummary).join('\n') : '- None',
    '',
    `## Next Planned`,
    '',
    planned.slice(0, 10).length ? planned.slice(0, 10).map(taskSummary).join('\n') : '- None',
  ].join('\n');
}
...
function doctorJson({ includeFixes = false } = {}) {
  ...
  const result = {
    ...
    git,
    files: {
      board: fs.existsSync(paths.boardPath),
      journal: fs.existsSync(paths.journalPath),
      messages: fs.existsSync(paths.messagesPath),
      runtime: fs.existsSync(paths.runtimeRoot),
      tasks: fs.existsSync(paths.tasksRoot),
    },
  };
...
function runDoctorJson(argv) {
  const includeFixes = argv.includes('--fix');
  console.log(JSON.stringify(doctorJson({ includeFixes }), null, 2));
  return includeFixes ? 0 : (doctorJson().ok ? 0 : 1);
}
...
function runDoctorFix() {
  ...
  if (fixes.length) {
    ...
    console.log('doctor --fix: nothing to fix');
  }
  return 0;
}
...
function runConfigValidation({ json = false } = {}) {
  ...
  if (json) {
    ...
    if (result.valid) console.log(`Config OK: ${normalizePath(configPath) || configPath}`);
      ...
      console.error(`Config invalid: ${normalizePath(configPath) || configPath}`);
...
function runLifecycle(commandName, argv, coordinatorScriptPath) {
  ...
  if (!agentId || !taskId) {
    console.error(`Usage: ${commandName} <agent-id> <task-id> [message] [--paths path[,path...]]`);
    return 1;
  }

  const message = rest.filter((entry) => !entry.startsWith('--')).join(' ').trim();
  const node = process.execPath;
  const base = [coordinatorScriptPath];
  const run = (args) => spawnSync(node, [...base, ...args], { cwd: ROOT, stdio: 'inherit', env: process.env }).status ?? 1;
  ...
  if (commandName === 'start') {
    ...
    if (message) return run(['progress', agentId, taskId, message]);
    return 0;
  }
  ...
  if (commandName === 'handoff-ready') {
    return run(['handoff', agentId, taskId, message || 'Ready for handoff.']);
  }
...
export async function runCommandLayer({ coordinatorScriptPath, importCore }) {
  ...
  if (commandName === 'doctor' && commandArgs.includes('--json')) status = runDoctorJson(commandArgs);
    ...
    if (commandArgs.includes('--json')) {
      ...
      console.log(buildBoardSummary({ forChat: commandArgs.includes('--for-chat') }));
    }
  } else if (VALID_LIFECYCLE_COMMANDS.has(commandName)) {
    status = runLifecycle(commandName, commandArgs, normalizedCoordinatorPath);
  }
